...
import { DataResult } from '@dooksa/ds-scripts'
import { deepClone, uuid } from '@dooksa/utils'
import { DsDataSchemaException, DsDataValueException } from './utils/error.js'
import { dsOperator, dsList } from './index.js'

/**
 * @typedef {import('@dooksa/ds-scripts/src/types.js').DsSetDataOptions} DsSetDataOptions
 * @typedef {import('@dooksa/ds-scripts/src/types.js').DsSchema} DsSchema
...
export default createPlugin('dsData', ({ defineActions, defineActionSchema, defineContextProperties }) => {
    ...
    if (schema.id) {
      ...
      if (!id) {
        if (schema.id.default) {
          ...
          return prefix + actions.generateId() + suffix
        }
      }
    }
  ...
  // this._schemaObjectPatternProperties
